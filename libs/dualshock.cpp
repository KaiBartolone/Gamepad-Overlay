...
hid_device *Dualshock::get_device(unsigned short id)
{
...
hid_device *Dualshock::get_device(std::string product)
{
    struct hid_device_info *devs, *cur_dev;
...
public:
    ...
    uint8_t small_rumble = 0;
    uint8_t big_rumble = 0;
    uint8_t led_r = 0;
    uint8_t led_g = 0;
    uint8_t led_b = 0;
    ...
    uint8_t factory_stick_cal[0x12];
    uint8_t device_colours[0xC];
    uint8_t user_stick_cal[0x16];
    ...
    //uint32_t crc_32(uint8_t* buf, int length) {
    ...
    void enable_gyro_ds4_bt(uint8_t *buf, int bufLength)
    {
        // enable gyro?
...
public:
    ...
    bool hid_exchange(hid_device *handle, uint8_t *buf, int len)
    {
        if (!handle)
            return false;
    ...
    void rumble(int frequency, int intensity)
    {
        ...
        // set non-blocking:
        hid_set_nonblocking(this->handle, true);
    ...
    bool get_switch_controller_info()
    {
        ...
        if (!get_spi_data(0x6080, 0x6, sensor_model))
        {
            return false;
        }
        if (!get_spi_data(0x6086, 0x12, stick_model))
        {
            return false;
        }
        if (!get_spi_data(0x6098, 0x12, &stick_model[0x12]))
        {
            return false;
        }
        ...
        //hex_dump(reinterpret_cast<uint8_t*>(sensor_cal[0]), 6);
        //hex_dump(reinterpret_cast<uint8_t*>(sensor_cal[1]), 6);

        return true;
    }

    void enable_IMU(uint8_t *buf, int bufLength)
    {
    ...
    void set_ds4_rumble_light(uint8_t smallRumble, uint8_t bigRumble,
                              uint8_t colourR,
                              uint8_t colourG,
                              uint8_t colourB)
    {
    ...
    void set_ds4_rumble_light_usb(uint8_t smallRumble, uint8_t bigRumble,
                                  uint8_t colourR,
                                  uint8_t colourG,
                                  uint8_t colourB)
    {
    ...
    void set_ds4_rumble_light_bt(uint8_t smallRumble, uint8_t bigRumble,
                                 uint8_t colourR,
                                 uint8_t colourG,
                                 uint8_t colourB)
    {
...
int Dualshock::send_command(int command, uint8_t *data, int len)
{
    uint8_t buf[0x40];
    memset(buf, 0, 0x40);
    ...
    buf[this->usb ? 0x8 : 0x0] = command;
    if (data != nullptr && len != 0)
        memcpy(buf + (this->usb ? 0x9 : 0x1), data, len);

    int res = hid_exchange(this->handle, buf, len + (this->usb ? 0x9 : 0x1));
...
void Dualshock::release_bt()
{
    init_bt();
}

void Dualshock::release_usb()
{
    uint8_t buf[40];
    memset(buf, 0, 40);
...
void Dualshock::enable_gyro(uint8_t *buf, int bufLength)
{
    // enable gyro?
    // assume everything already zeroed
    buf[0] = 0xa3;
...
int Dualshock::hid_exchange(hid_device *handle, uint8_t *buf, size_t len)
{
    if (!handle)
        return false;

    int res = hid_write(handle, buf, len);
    res = hid_read_timeout(handle, buf, 0x40, 1000);
